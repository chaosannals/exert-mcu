...
# MOSI 或 SDA 数据线
# RES 重启信号
# DC 引脚用来切换 数据（Data） 命令(Command)   0 传命令，1 传数据
# CS 选择
# BLK 背光
...
class TFT(object):
    ...
    def __init__(self, spi, aDC, aReset, aCS):
        ...
        self.reset = machine.Pin(
            aReset, machine.Pin.OUT, machine.Pin.PULL_DOWN)
...
#   @micropython.native
    def _setwindowpoint(self, aPos):
        ...
        self._writecommand(TFT.CASET)  # 列地址设置（Column address set）
        ...
        self._writecommand(TFT.RASET)  # 行地址设置（Row address set）
...
#   @micropython.native
    ...
    def initg(self):
        ...
        self._writecommand(TFT.SWRESET)  # 软重置 Software reset.
        time.sleep_us(150)
        self._writecommand(TFT.SLPOUT)  # 离开睡眠模式 out of sleep mode.
        time.sleep_us(255)
